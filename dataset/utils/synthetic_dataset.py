#this code is mainly modified from Superpoint-pytorch[https://github.com/shaofengzeng/SuperPoint-Pytorch]
import cv2 as cv
import numpy as np
import math
...
def draw_lines(img, nb_lines=10):
    ...
    """
    key_points = np.empty((0, 2), dtype=np.int)
    ...
    for i in range(num_lines):
        ...
        for x, y in bresenham(x1, y1, x2, y2):
            # print(x, y)
            points = np.concatenate([points, np.array([[x, y]])], axis=0)
        points = np.concatenate([points, np.array([[x1, y1], [x2, y2]])], axis=0)
        key_points =np.concatenate([key_points, np.array([[x1, y1], [x2, y2]])], axis=0)
    # print(points.shape)
...
def draw_multiple_polygons(img, max_sides=8, nb_polygons=30, **extra):
    ...
    rads = []
    points = np.empty((0, 2), dtype=np.int)
    keypoints = np.empty((0, 2), dtype=np.int)
    background_color = int(np.mean(img))
    for i in range(nb_polygons):
        ...
        for i in range(len(new_points)):
            ...
            end = new_points[(i + 1) % len(new_points)]
            for x, y in bresenham(start[0], start[1], end[0], end[1]):
                # print("像素位置：({}, {})".format(x, y))
                points = np.concatenate([points, np.array([[x, y]])], axis=0)
...
def draw_ellipses(img, nb_ellipses=20):
    ...
    for i in range(nb_ellipses):
        ...
        for theta in range(360):
            ...
            px = int(np.round(x + ax * np.cos(rad) * np.cos(np.deg2rad(angle)) - ay * np.sin(rad) * np.sin(np.deg2rad(angle))))
            py = int(np.round(y + ax * np.cos(rad) * np.sin(np.deg2rad(angle)) + ay * np.sin(rad) * np.cos(np.deg2rad(angle))))
            points = np.concatenate([points, np.array([[px, py]])], axis=0)
    return points,keypoints
...
def draw_star(img, nb_branches=6):
    ...
    points = np.array([[int(x + max(random_state.rand(), 0.3) * rad * math.cos(a)),
                       ...
                       for a in angles])
    keypoints =  np.concatenate(([[x, y]], points), axis=0)
    for pt in points:
         for x, y in bresenham(pt[0],pt[1], x, y):
            points = np.concatenate([points, np.array([[x, y]])], axis=0)
...
def draw_stripes(img, max_nb_cols=13, min_width_ratio=0.04,
    ...
    for i in range(col):
        ...
        cv.fillConvexPoly(img, np.array([(warped_points[i, 0],
                          ...
                          color)
    #superedge
    temp_points = np.empty((0, 2), dtype=np.int)
    ...
    for _ in range(nb_rows):
        ...
        cv.line(img, (warped_points[row_idx + col_idx1, 0],
                ...
                color, thickness)
        for x, y in bresenham( warped_points[row_idx + col_idx1, 0],
                      warped_points[row_idx + col_idx1, 1],
                warped_points[row_idx + col_idx2, 0],
                 warped_points[row_idx + col_idx2, 1]):
            temp_points = np.concatenate([temp_points, np.array([[int(x), int(y)]])], axis=0)
    ...
    for _ in range(nb_cols):
        ...
        cv.line(img, (warped_points[col_idx, 0],
                ...
                color, thickness)
        for x, y in bresenham(warped_points[col_idx, 0], warped_points[col_idx, 1],
                            warped_points[col_idx + col + 1, 0], warped_points[col_idx + col + 1, 1]):
            temp_points = np.concatenate([temp_points, np.array([[int(x), int(y)]])], axis=0)

    # Keep only the points inside the image

    temp_points = keep_points_inside(temp_points,img.shape[:2])
    points = np.concatenate([temp_points, points], axis=0)
    return points
...
def draw_cube(img, min_size_ratio=0.2, min_angle_rot=math.pi / 10,
    ...
    for i in [0, 1, 2]:
        ...
        tmp_points =  cube[faces[i]]
        for k in range(len(tmp_points)):
            start = tmp_points[k]
            end = tmp_points[(k + 1) % len(tmp_points)]
            for x, y in bresenham(start[0], start[1], end[0], end[1]):
                points = np.concatenate([points, np.array([[x, y]])], axis=0)
    ...
    keypoints = keep_points_inside(keypoints, img.shape[:2])
    return points,keypoints
...
def gaussian_noise(img):
    ...
    cv.randu(img, 0, 255)
    return np.empty((0, 2), dtype=np.int),np.empty((0, 2), dtype=np.int)
