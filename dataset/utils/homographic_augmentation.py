...
def homographic_aug_pipline(img, pts, kp_kpts ,config, device='cpu'):
    """
    :param img: [1,1,H,W]
    :param pts:[N,2]
    :param config:parameters
    ...
    warped_points = warp_points(pts, homography, device=device)
    if kp_kpts != None:
        warped_kp_points = warp_points(kp_kpts, homography, device=device)
        warped_kp_points = filter_points(warped_kp_points, image_shape, device=device)
        warped_kp_points_map = compute_keypoint_map(warped_kp_points, img.shape[2:], device=device)
    else:
        warped_kp_points = None
        warped_kp_points_map = None
    warped_points = filter_points(warped_points, image_shape, device=device)
    warped_points_map = compute_keypoint_map(warped_points, img.shape[2:], device=device)

    if warped_kp_points_map is not None:
        return {'warp': {'img': warped_image.squeeze(),
                        'kpts': warped_points,
                        'kp_kpts': warped_kp_points,
                        'kpts_map': warped_points_map.squeeze(),  # some point maybe filtered
                        'kp_kpts_map': warped_kp_points_map.squeeze(),
                        'mask': warped_valid_mask.squeeze(),
                        'pri_H': image_shape[0],
                        'pri_W': image_shape[1],
                        },
